...
def test_login_success(db, test_user):
    ...
    response = client.post(
        "/api/v1/auth/login",
        json={
            "username_or_email": "testuserauth",
...
def test_login_admin_requires_password_change(db):
    """Test that admin user requires password change on first login"""
    # Admin user should exist from migration
    response = client.post(
        "/api/v1/auth/login",
        json={
...
def test_get_current_user_with_token(db, test_user):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["username"] == "testuserauth"
